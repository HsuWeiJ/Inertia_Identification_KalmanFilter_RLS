...
float RLS_Inertia_min = 0.5;
float RLS_Inertia_max = 1.5;
...
float RLS_To_Kalman_B = BM;
float Speed_err_threshold = 1e-07;

//拖載啟動
char Inertia_Reset = 0;
...
float tt1[9] = {4,12,-16,12,37,-43,-16,-43,98};
...
//	Kalman_Ini_2X2(&Kalman2X2_Handler);
	...
	//測試
	Kalman_Calculate_U(&UKalman_Handler , 1 ,  1, RLS_To_Kalman_J );

//	GPIO_WritePin(GPIO25,TRUE);
//	Kalman_Calculate_U(&UKalman_Handler , motor1.MechTheta * TWO_PI ,  motor1.pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J );
//	GPIO_WritePin(GPIO25,FALSE);
//	cholesky(tt1, 3);
//	Matrix_Transpose(tt1, tt2, 3);
//	Matrix_Dot_Product(tt2, 0.5, UKalman_Handler.Sigma, 9);
//	UKalman_Handler.Sigma[3]=0;UKalman_Handler.Sigma[6]=0;UKalman_Handler.Sigma[7]=0;
//    Matrix_Add(UKalman_Handler.X[0], UKalman_Handler.Sigma+FIRST_COLUMN, UKalman_Handler.X[1], 3);
//	Matrix_Add(UKalman_Handler.X[0], UKalman_Handler.Sigma+SECOND_COLUMN, UKalman_Handler.X[2], 3);
//    Matrix_Add(UKalman_Handler.X[0], UKalman_Handler.Sigma+THIRD_COLUMN, UKalman_Handler.X[3], 3);
//    Matrix_Minor(UKalman_Handler.X[0], UKalman_Handler.Sigma+FIRST_COLUMN, UKalman_Handler.X[4], 3);
//    Matrix_Minor(UKalman_Handler.X[0], UKalman_Handler.Sigma+SECOND_COLUMN, UKalman_Handler.X[5], 3);
//    Matrix_Minor(UKalman_Handler.X[0], UKalman_Handler.Sigma+THIRD_COLUMN, UKalman_Handler.X[6], 3);
//
//    int i;
//    //X_Priori=A*X+B.*u;
////    mpy_SP_RMxRM(UKalman_Handler.X, UKalman_Handler.I, UKalman_Handler.X, 3, 3, 7);   // Tem_nX1_1 : A*X
//    for( i=0;i<7;i++)
//    {
//        mpy_SP_RMxRM(UKalman_Handler.Tem_nX1_1, UKalman_Handler.I, UKalman_Handler.X[i], 3, 3, 1);   // Tem_nX1_1 : A*X
//        Matrix_Dot_Product(UKalman_Handler.B, 1, UKalman_Handler.Tem_nX1_2, 3);                 // Tem_nX1_2 : B.*u
//        Matrix_Add(UKalman_Handler.Tem_nX1_1, UKalman_Handler.Tem_nX1_2, UKalman_Handler.X[i] , 3);
//    }




//	mpy_SP_RMxRM(UKalman_Handler.B, UKalman_Handler.I, UKalman_Handler.X[1], 3, 3, 1) ;

//    Matrix *x = (Matrix*) calloc(7,sizeof(Matrix));
//    float B_data[3] = {1,2,3};
//    Matrix_Generate(&x[1],3,B_data);
//    float A_data[3] = {3,2,1};
//    Matrix_Generate(&x[2],3,A_data);
//    float C_data[1] = {0};
//    Matrix ttt;
//    Matrix_Generate(&ttt,1,C_data);
//
//    float I_data[9] = {1,0,0,0,1,0,0,1,1};
//    Matrix I;
//    Matrix_Generate(&I,9,I_data);
//
//    float D_data[3] = {1,0,0};
//    Matrix D;
//    Matrix_Generate(&D,3,D_data);
//    mpy_SP_RMxRM(ttt, x[1], x[2], 1, 3, 1) ;
...
//    Matrix x2 =x[2];
//    Matrix_Generate(&ttt,1,C_data);
//    mpy_SP_RMxRM(D, I, x[2], 3, 3, 1) ;
//    mpy_SP_RMxRM(x[1], I, x[2], 3, 3, 1) ;

//    float aa[9]={1,0,0,0,1,0,0,1,1};
//    float bb[3]={1,1,1};
//    float cc[3]={0};
//    mpy_SP_RMxRM(bb, aa, bb, 3, 3, 1) ;
...
// -----------------------------------------------------------------------------------
    ...
    {
        Inertia_Reset = 0;
        RLS_Handler.Theda[0] = -0.999;
        RLS_Handler.Theda[1] = 1.13;
    }
    ...
    if(RLS_Flag)
    {
        ...
        {
            RLS_cnt = 0;
            RLS_To_Kalman_J = fminf(fmaxf(RLS_Inertia.Out,RLS_Inertia_min*J),RLS_Inertia_max*J);
        }
...
//    GPIO_WritePin(GPIO25,TRUE);
    ...
    RLS_Inertia.Out = RLS_Handler.Terminal.Inertia;

#if (BUILDLEVEL == LEVEL4 || BUILDLEVEL == LEVEL6)
    if(Adaptive_On)
    {
...
//        SPIDAC_write_dac_channel(3, (Kalman2X2_Handler.Terminal.speed/(2*PI*BASE_FREQ))*2047 * DAC_Gain_D + 2048);
        ...
        DAC_Gain_A = 50; DAC_Gain_B = 2; DAC_Gain_C = 570; DAC_Gain_D = 570;   // Brake
        SPIDAC_write_dac_channel(0, (motor->speed_est.term.Enhanced_SpeedEst_pu)*2047 * DAC_Gain_A + 2048);
        SPIDAC_write_dac_channel(1, (Kalman3X3_Handler.KalmanToRls_TL *0.1)*2047 * DAC_Gain_B + 2048);
...
//        DAC_Gain_A = 50; DAC_Gain_B = 50; DAC_Gain_C = 5000; DAC_Gain_D = 5000;   //No Brake
        DAC_Gain_A = 50; DAC_Gain_B = 50; DAC_Gain_C = 1; DAC_Gain_D = 2000;   // Brake
        SPIDAC_write_dac_channel(0, (motor->pi_spd.term.Ref)*2047 * DAC_Gain_A + 2048);
        SPIDAC_write_dac_channel(1, (motor->speed_est.term.Enhanced_SpeedEst_pu)*2047 * DAC_Gain_B + 2048);
        SPIDAC_write_dac_channel(2, (RLS_Handler.Switch *0.5)*2047 * DAC_Gain_C + 2048);
        SPIDAC_write_dac_channel(3, (RLS_Inertia.Out)*2047 * DAC_Gain_D + 2048);
#endif
    }
...
// -----------------------------------------------------------------------------------
    if(RLS_Flag)
    {
        ...
        {
            RLS_cnt = 0;
            RLS_To_Kalman_J = fminf(fmaxf(RLS_Inertia.Out,0.82*J),1.18*J);
        }
        Kalman3X3_Calculate(&Kalman3X3_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J );
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J);
//        Kalman3X3_Calculate(&Kalman3X3_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
...
//    }
        ...
        DAC_Gain_A = 50; DAC_Gain_B = 50; DAC_Gain_C = 2000; DAC_Gain_D = 2000;   // Brake
        SPIDAC_write_dac_channel(0, (motor->pi_spd.term.Ref)*2047 * DAC_Gain_A + 2048);
        SPIDAC_write_dac_channel(1, (motor->speed_est.term.Enhanced_SpeedEst_pu)*2047 * DAC_Gain_B + 2048);
        SPIDAC_write_dac_channel(2, (J)*2047 * DAC_Gain_C + 2048);
        SPIDAC_write_dac_channel(3, (RLS_Inertia.Out)*2047 * DAC_Gain_D + 2048);
...
// -----------------------------------------------------------------------------------
    ...
    if(RLS_Flag)
    {
        if(++RLS_cnt >= RLS_prescaler)
        {
            RLS_cnt = 0;
            RLS_To_Kalman_J = fminf(fmaxf(RLS_Inertia.Out,RLS_Inertia_min*J),RLS_Inertia_max*J);
//            RLS_To_Kalman_B = fminf(fmaxf(RLS_Handler.Terminal.Friction,RLS_Inertia_min*BM),RLS_Inertia_max*BM);
            RLS_To_Kalman_B = BM;
        }
        Kalman_Calculate_U(&UKalman_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J, RLS_To_Kalman_B );
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J);
//        Kalman3X3_Calculate(&Kalman3X3_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
    }
...
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
    ...
    Kalman_e.Input = UKalman_Handler.e;
    LPF_MARCRO(Kalman_e)

    Kalman3X3_TL[1] = Kalman3X3_TL[0];
//    Kalman3X3_TL[0] = UKalman_Handler.Terminal.TL;
    Kalman3X3_TL[0] = UKalman_Handler.KalmanToRls_TL;
//    Kalman2X2_TL[1] = Kalman2X2_TL[0];
//    Kalman2X2_TL[0] = Kalman2X2_Handler.Terminal.TL;
    ...
    SpeedCmd0_1 = fabsf(SpeedCmd[0] - SpeedCmd[1]);
//if(fabsf(Speed_CMD - UKalman_Handler.Terminal.speed / TWO_PI / BASE_FREQ) < Speed_err_threshold)
    if(fabsf(SpeedCmd[0] - SpeedCmd[1]) < 0.000001)
    {

//        GPIO_WritePin(GPIO25,TRUE);
        UKalman_Handler.KalmanToRls_TL = UKalman_Handler.Terminal.TL;
//        Kalman2X2_Handler.KalmanToRls_TL = Kalman2X2_Handler.Terminal.TL;
    }
    else
    {
...
//        Kalman2X2_Handler.KalmanToRls_TL = Kalman2X2_Handler.Terminal.TL;
    ...
    Te = motor->pi_iq.Ref * BASE_CURRENT * KT;
#if KALMAN3X3_OR_2X2
    Te_TL_Fbk = Iq_FBK.Out * BASE_CURRENT * KT - UKalman_Handler.KalmanToRls_TL;
//    Te_TL_Cmd = motor->pi_iq.Ref * BASE_CURRENT * KT - Kalman3X3_Handler.KalmanToRls_TL;
    Te_TL_Cmd = motor->pi_iq.Ref * BASE_CURRENT * KT * RLS_Gain - UKalman_Handler.KalmanToRls_TL;
#else
    Te_TL_Fbk = Iq_FBK.Out * BASE_CURRENT * KT - Kalman2X2_Handler.KalmanToRls_TL;
    Te_TL_Cmd = motor->pi_iq.Ref * BASE_CURRENT * KT - Kalman2X2_Handler.KalmanToRls_TL;
#endif
...
//    GPIO_WritePin(GPIO25,FALSE);
    ...
    RLS_Calculate(&RLS_Handler, &UKalman_Handler, UKalman_Handler.Terminal.speed, Te_TL_Cmd, Estimated_Acc.Out, RLS_Start_Flag);
//    RLS_Calculate(&RLS_Handler, &UKalman_Handler, motor->speed.Speed / (POLES/2) * TWO_PI * BASE_FREQ, Te_TL_Cmd, Estimated_Acc.Out, RLS_Start_Flag);

//    RLS_Calculate(&RLS_Handler, &UKalman_Handler, motor->speed_est.term.Enhanced_SpeedEst_pu *BASE_FREQ * TWO_PI, Te_TL_Cmd, Estimated_Acc.Out,RLS_Start_Flag);
//    RLS_Calculate(&RLS_Handler, &UKalman_Handler, motor->speed.Speed / (POLES/2) *BASE_FREQ * TWO_PI, Te_TL_Cmd, Estimated_Acc.Out, RLS_Start_Flag);
#else

    RLS_Calculate(&RLS_Handler, &UKalman_Handler,Kalman2X2_Handler.Terminal.speed, Te_TL_Cmd, Estimated_Acc.Out, RLS_Start_Flag);
#endif
//    GPIO_WritePin(GPIO25,TRUE);
   ...
   if (++motor->SpeedLoopCount >= motor->SpeedLoopPrescaler)
     {
        ...
        if(SpeedFBK_Cnt >= 2000 && motor->lsw==2) {
//            motor->pi_spd.term.Fbk = motor->speed_est.term.Enhanced_SpeedEst_pu;
//            motor->pi_spd.term.Fbk = UKalman_Handler.Terminal.speed / TWO_PI / BASE_FREQ;
            motor->pi_spd.term.Fbk = motor->speed.Speed / (POLES/2);
            motor1.pi_spd.param.Kp = _IQ(1.2 * Speed_Kp_Gain);   //1.2 * Speed_Kp_Gain(1.0)  for speed bw = 30 HZ
            motor1.pi_spd.param.Ki = _IQ(0.5 * Speed_Ki_Gain);   //0.5 * Speed_Ki_Gain(1.0)  for speed bw = 30 HZ
        }
...
//------------------------------------------------------------------------------
    ...
    if(DAC_Switch == 1){
        DAC_Gain_A = 100; DAC_Gain_B = 2; DAC_Gain_C = 2; DAC_Gain_D = 2;
        SPIDAC_write_dac_channel(0, (Kalman_e.Out)*2047 * DAC_Gain_A + 2048);             //VoutA on SPIDAC Chip
        SPIDAC_write_dac_channel(1, (UKalman_Handler.KalmanToRls_TL *0.1)*2047 * DAC_Gain_B + 2048);    //VoutB on SPIDAC Chip
        SPIDAC_write_dac_channel(2, (UKalman_Handler.Terminal.TL *0.1)*2047 * DAC_Gain_C + 2048);          //VoutC on SPIDAC Chip
//        SPIDAC_write_dac_channel(3, (Kalman2X2_Handler.Terminal.TL *0.1)*2047 * DAC_Gain_D + 2048);          //VoutD on SPIDAC Chip
    }
    else if(DAC_Switch == 2){
        SPIDAC_write_dac_channel(0, (motor->pi_iq.Ref)*2047 * DAC_Gain_A + 2048);
...
//    }
        ...
        SPIDAC_write_dac_channel(0, (motor->pi_spd.term.Ref)*2047 * DAC_Gain_A + 2048);
        SPIDAC_write_dac_channel(1, (motor->speed_est.term.Enhanced_SpeedEst_pu)*2047 * DAC_Gain_B + 2048);
        SPIDAC_write_dac_channel(2, (UKalman_Handler.Terminal.speed/(2*PI*BASE_FREQ))*2047 * DAC_Gain_C + 2048);
//        SPIDAC_write_dac_channel(3, (Kalman2X2_Handler.Terminal.speed/(2*PI*BASE_FREQ))*2047 * DAC_Gain_D + 2048);
    }
    else if(DAC_Switch == 5){
        DAC_Gain_A = 50; DAC_Gain_B = 2; DAC_Gain_C = 570; DAC_Gain_D = 570;   // Brake
        SPIDAC_write_dac_channel(0, (motor->speed_est.term.Enhanced_SpeedEst_pu)*2047 * DAC_Gain_A + 2048);
        SPIDAC_write_dac_channel(1, (UKalman_Handler.KalmanToRls_TL *0.1)*2047 * DAC_Gain_B + 2048);
        SPIDAC_write_dac_channel(2, (J)*2047 * DAC_Gain_C + 2048);
        SPIDAC_write_dac_channel(3, (RLS_Inertia.Out)*2047 * DAC_Gain_D + 2048);
    }
    else if(DAC_Switch == 6){
...
// -----------------------------------------------------------------------------------
    ...
    if(RLS_Flag)
    {
        ...
        {
            RLS_cnt = 0;
            RLS_To_Kalman_J = fminf(fmaxf(RLS_Inertia.Out,0.8*J),1.2*J);
        }
        Kalman_Calculate_U(&UKalman_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J );
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, RLS_To_Kalman_J);
//        Kalman3X3_Calculate(&Kalman3X3_Handler , motor->MechTheta * TWO_PI ,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
//        Kalman2X2_Calculate(&Kalman2X2_Handler , motor->speed.Speed / (POLES/2) * BASE_FREQ * TWO_PI,  motor->pi_iq.Ref * BASE_CURRENT * KT, J);
    }
